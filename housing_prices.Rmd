...
for (variable_name in categorical_predictors) {
    ...
    train_data <- encode(train_data, variable_name, target_mean)
}
...
An iterative process was used to tune the stepwise regression:
  ...
  2. Drop variables that contain many missing values.  In the first iteration,
     ...
     well.   The remaining numerical and encoded categorical predictors had no
     missing values.

  3. Execute the stepwise regression using a data set that includes all
     numerical and encoded categorical predictors, together with dummy variables
...
impute_mean <- function(vec) {
    ...
    vec[bool_index] <- vec_mean
}
preprocess <- function(data, drop_categorical = TRUE, verbose = FALSE) {
    # For simplicity drop MasVnrArea, since it is not used in the linear model.
    data <- select(data, -LotFrontage, -MasVnrArea) %>%
        mutate(MSSubClass = as.character(MSSubClass)) %>%
        define_dummies()

    # Impute missing numerical values by the mean.
    for (variable_name in intersect(numeric_predictors, colnames(data))) {
        ...
        if (missing_count > 0) {
            ...
            data[[variable_name]] <- impute_mean(data[[variable_name]])
        }
    }
    ...
    for (variable_name in categorical_predictors) {
        # The assign_encoding function is defined in the script encode.R.
        data <- assign_encoding(data, variable_name, target_mean = target_mean,
                                method = 'mean')
        if (drop_categorical) {
...
# the room-mean-square logarithmic error, taking into account the fact that the
# target was transformed by taking the log.
...
get_rmse <- function(predicted, actual) {
    ...
    num_rows <- length(predicted)
    mse <- sum((exp(predicted) - exp(actual))^2) / num_rows
    return(sqrt(mse))
}

get_rmsle <- function(predicted, actual) {
    check_vector_lengths(predicted, actual)
    num_rows <- length(predicted)
    msle <- sum((log(exp(predicted) + 1) - log(exp(actual) + 1))^2) / num_rows
    return(sqrt(msle))
}
...
into three levels by importance.
  ...
  - In reporting the results, suppress the technical distinction between the original
    variables and the transformed or encoded variables used in the multilinear
    model.
