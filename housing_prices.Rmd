...
encode_predictors <- function(data, method = 'mean', drop_categorical = TRUE) {
    ...
    if ('SalePrice' %in% colnames(data)) {
        ...
        for (variable_name in categorical_predictors) {
            ...
            if (drop_categorical) {
                data <- select(data, -all_of(variable_name))
            }
        }
    }
...
train_data <- encode_predictors(train_data)
...
```{r, stepwise-regression, cache=FALSE, echo=FALSE}
...
```{r, lasso, cache=FALSE, echo=FALSE}
get_glmnet_inputs <- function(data, predictors, outliers = integer()) {
    data <- data %>%
        filter(!(Id %in% outliers))
...
```{r, linear-score, cache=FALSE, echo=FALSE}
...
report_metrics <- function(glmnet_inputs) {
...
into three levels by importance.
  ...
  - Note the training process includes a random element, and so the relative
    ...
    somewhat different than the hard-coded values shown in Sec 1.1.

### Primary variables {.tab-display .unnumbered}

<div class="code-block">

```{r, primary, cache=FALSE}
...
The plots for the multilinear model show large outliers.  Test whether a
random-forest model can improve accuracy.

## Feature selection {.tabset}

### Summary {.unnumbered}

Use a process inspired by stepwise regression to select a feature set.
...
Prepare data:
  ...
  - The variables LotFrontage and MasVnrArea were dropped, since these
    variables each contain a relatively large number of missing variables.
...
large ntree needs to be to give stable results.
  ...
  - Both root-mean-square logarithmic error (RMSLE) and the feature importance
    (evaluated by permunting predictor variables) should be stable.  
  - A loop with different values of ntree found that for a model including all
    predictors in the prepared data, the choice of least-important feature was
    the same for all values of ntree $\geq 5000$.  Changes in RMSLE as ntree
    was increased above 5000 were negligible.
...
after data preparation, eliminate predictors in a stepwise process:
  ...
  - Find the RMSLE for the model that has the optimal value of mtry.  Note
    that the predictions used to calculate RMSLE are based on out-of-bag
    samples.
...
preprocess_tree <- function(data, encoding_method = 'mean',
    ...
    data <- select(data, -Id, -LotFrontage, -MasVnrArea)

    # The target encoding that was explored for the linear model used
    ...
    # Encode categorical variables.  Note that encodings assigned to teeare
    # based on log(SalePrice), although transformations are not applied to any
...
all_tree_predictors <- colnames(test_data)
...
if (select_features_rf) {
    ...
    tree_features <- feature_sets[[selected_index]]
